...
translate([BoxLengthUnits*BoxUnits/2,1.8+springThickness/2,0])
    rotate([0,0,90])
        springSine3D(
            length=BoxLengthUnits*BoxUnits-1.7-springThickness,
            heigth=BoxHeight,
            width=BoxWidthUnits*BoxUnits-2,
            windings=4,
            steps=10,
            wireDiameter=springThickness,
            fn=40
        );
...
*/
module line3D(start, end,d=1,h=10, fn=40) {
  hull() {
