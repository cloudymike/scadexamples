...
// baseplate styles
style_plate = 5; // [0: thin, 1:weighted, 2:skeletonized, 3: screw together, 4: screw together minimal, 5 screw together maximized]
...
module gridfinityBaseplate(gridx, gridy, length, dix, diy, sp, sm, sh, fitx, fity) {
    ...
    difference() {
        ...
        if (sp == 3 || sp ==4 || sp ==5) cutter_screw_together(gx, gy, off);
        if (sp ==5) cutter_screwandnut(gx,gy,off);
...
module cutter_screwandnut(gx, gy, off) {

    screwandnut(gx, gy);
    rotate([0,0,90])
    screwandnut(gy, gx);

    module screwandnut(a, b) {
        copy_mirror([1,0,0])
        translate([a*l_grid/2, 0, -off/2])
        pattern_linear(1, b, 1, l_grid)
        pattern_linear(1, n_screws, 1, d_screw_head + screw_spacing)
