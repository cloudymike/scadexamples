...
// Main parameters
pcbLength = 89;           // Length of each PCB (mm)
gapBetweenPcbs = 1;       // Gap between each PCB and the next (mm)
multiRow = true;          // Set to true to allow multiple rows of PCBs
gapBetweenRows = 5;       // If multiRow enabled, gap between each row (mm)

// Tweaks
pcbThickness = 1.6;       // Thickness of each PCB (standard is 1.6)
thicknessClearance = 0.2; // Clearance added to pcbThickness to allow easy sliding in
lengthClearance = 1;      // Clearance added to pcbLength to allow easy sliding in
...
// ===== PCB Holder Implementation ===== //

// Main
difference()
{
  box();
  //cube(120); // Uncomment for 1/4 cross-section
}

// Non-user-modifiable pcbholder values
pcbHeight = gridz * 7;    // Height of PCBs slots (extends out of top of box
pcbZOffset = 6;           // How far PCBs are raised from bottom of box
border = 3;               // Border to take into account stacking lip

// Calculated pcbholder values
slotThickness = pcbThickness + thicknessClearance;
slotLength = pcbLength + lengthClearance;
slotInterval = slotThickness + gapBetweenPcbs;
howManyInARow = round((((gridy * length) - 2*border) / slotInterval));
totalRowLength = (howManyInARow-1) * slotInterval;
rowInterval = pcbLength + lengthClearance + gapBetweenRows;
howManyRows = round((((gridx * length) - 2*border) / rowInterval) - 0.5);
allRowsWidth = howManyRows * rowInterval;

echo("Boxlen ", ((gridx * length) - 2*border), " rowInterval ", rowInterval);
echo(howManyInARow, "in a row, row length" , totalRowLength);
echo(howManyRows, " rows");

//multiRowSlots();
module multiRowSlots()
{
  translate([-(allRowsWidth/2)+rowInterval/2,-(totalRowLength/2),0])
  {
    for(j = [0 : howManyRows-1])
    {
      for(i = [0 : howManyInARow-1])
      {
        translate([j*rowInterval,i * slotInterval,0])
          pcbSlot();
      }
    }
  }
...
module pcbSlots()
{
  translate([0,-(totalRowLength/2),0])
  {
    for(i = [0 : howManyInARow-1])
    {
      translate([0,i * slotInterval,0])
        pcbSlot();
    }
  }
}
...
module pcbSlot()
{
  translate([0,0,pcbHeight/2 + pcbZOffset])
    cube([slotLength, slotThickness, pcbHeight], center=true);
}

// ===== End PCB Holder Implementation ===== //
...
module box()
{
  difference()
  {
    ...
    // Subtract PCB slots from filled box
    if(multiRow)
    {
      multiRowSlots();
    }
